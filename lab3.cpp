...
struct ГазотранспортнаяСеть {
    ...
    void добавитьТрубу(Труба& труба) {
        if (колТруб < MAX_ТРУБ) {
            ...
            std::cout << "Достигнут лимит труб в сети!" << std::endl;
        }
    }

    // Функция для добавления компрессорной станции в сеть
    void добавитьСтанцию(КомпрессорнаяСтанция& станция) {
        if (колСтанций < MAX_СТАНЦИЙ) {
            станции[колСтанций++] = &станция;
        } else {
            std::cout << "Достигнут лимит станций в сети!" << std::endl;
        }
    }
